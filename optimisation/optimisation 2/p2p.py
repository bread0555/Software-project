class CodeBlock:
    ...
    def tab_to_spaces(self):
        self.i_p = [line.replace("\t", "    ") for line in self.i_p]
    ...
    def check_indents(self):
        ...
        indents = [len(line) - len(line.lstrip(" ")) for line in self.i_p]
        hcf = indents[0]
    ...
    def analyse(self):
        self.tab_to_spaces()
        self.check_indents()
        control_structures = ["IF", "WHILE", "FOR", "TRY", "CASE", "REPEAT", "FUNCTION", "CASEWHERE"]
        i = 0
    ...
    def translate_if(self, i_p):
        ...
        while i < len(i_p):
            if not i_p[i].startswith(self.indent * (self.indent_lvl + 1)):
                ...
                line = i_p[i].split()
                line = [replacements.get(word, word) for word in line]
                if constructor == "IF":
                    ...
                    o_p += CodeBlock(" ".join(line)).analyse().o_p
                i += 1
            else:
                start = i
                while i < len(i_p):
                    if not i_p[i].startswith(self.indent * (self.indent_lvl + 1)):
                        ...
                        break
                    i += 1
                o_p += CodeBlock(i_p[start:end]).analyse().o_p
        return o_p
    ...
    def translate_casewhere(self, i_p):
        ...
        while i < len(i_p):
            if not i_p[i].startswith(self.indent * (self.indent_lvl + 1)):
                ...
                line = [seq.strip() for seq in i_p[i].split(":")]
                if "TO" in line[0]:
                    ...
                    o_p.append(self.indent * self.indent_lvl + selector + variable + " == " + line[0] + ":")
                o_p += [self.indent * (self.indent_lvl + 1) + line for line in CodeBlock(line[1:]).analyse().o_p]
                selector = "elif "
                i += 1
            else:
                start = i
    ...
    def translate_print(self, i_p):
        return self.indent * self.indent_lvl + "print(" + " ".join(i_p.split()[1:]) + ")"
    ...
    def translate_set(self, i_p):
        return self.indent * self.indent_lvl + " ".join(i_p.split()[1:])

    def translate_call(self, i_p):
        return self.indent * self.indent_lvl + " ".join(i_p.split()[1:])

    def translate_append(self, i_p):
        operand = [seq.strip() for seq in " ".join(i_p.split()[1:]).split("TO")]
        return self.indent * self.indent_lvl + operand[1] + ".append(" + operand[0] + ")"
